...
describe('ShortenForm', () => {
  ...
  test('it does not accept invalid input', async () => {
    ...
    expect(screen.getByRole('button', { name: /shorten/i })).toHaveAttribute(
      'disabled'
    );
  });
  ...
  test('it renders backend errors on the page', async () => {
    ApiClient.prototype.shortenUrl = jest
      .fn()
      .mockRejectedValueOnce('invalid input data');
